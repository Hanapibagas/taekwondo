...
class IklanController extends Controller
{
    public function index(Request $request)
    {
      if ($request->ajax()) {
        ...
        return Datatables::of($data)
          ->addColumn('gambar', function ($data) {
            $url = asset('storage/images/iklan/' . $data->images);
      ...
      return view('dashboard.admin.iklan.index');
    }
    ...
    public function store(Request $request)
    {
      ...
      $data_kebijakan = Iklan::where('id', $id_kebijakan)->first();
      ...
      if ($id_kebijakan == NULL && !$request->file('image')) {
        ...
        Storage::disk('local')->putFileAs('public/images/iklan/', $files, $file_name);
      } elseif ($id_kebijakan == $data_kebijakan->id && $request->file('image')) {
        Storage::delete('/public/images/iklan/' . $data_kebijakan->images);
        $file_name = date('YmdHis') . "." . $files->getClientOriginalExtension();
        Storage::disk('local')->putFileAs('public/images/iklan/', $files, $file_name);
      } else {
    ...
    {
      $post = Iklan::findOrFail($id);
      Storage::delete('/public/images/iklan/' . $post->images);
      $post->delete();
      return response()->json($post);
    }
}
